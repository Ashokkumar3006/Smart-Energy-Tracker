...
class EmailService:
    def __init__(self):
        ...
        if not self.config_valid:
            print("⚠️  Email Configuration Issues Detected:")
            if not self.sender_email:
                print("   - SENDER_EMAIL not set")
            elif '@' not in self.sender_email:
                print(f"   - SENDER_EMAIL appears invalid: {self.sender_email}")
            if not self.sender_password:
                print("   - SENDER_PASSWORD not set")
            elif len(self.sender_password) != 16 and 'gmail.com' in self.smtp_server:
                print(f"   - Gmail requires 16-character App Password (current: {len(self.sender_password)} chars)")
    ...
    def _validate_config(self) -> bool:
        ...
        if 'gmail.com' in self.smtp_server:
            if len(self.sender_password) != 16:
                print(f"⚠️  Gmail App Password should be 16 characters long (current: {len(self.sender_password)})")
    ...
    def test_connection(self) -> Dict:
        ...
        if not self.config_valid:
            return {
                "success": False,
                "error": "Invalid email configuration. Please check your .env file.",
                "troubleshooting": [
                    "1. Check SENDER_EMAIL is set correctly",
                    "2. Check SENDER_PASSWORD is a 16-character Gmail App Password",
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Successfully connected to {self.smtp_server} with {self.sender_email}"
            }
            ...
            if "Username and Password not accepted" in error_msg:
                return {
                    "success": False,
                    "error": "Gmail authentication failed. Please ensure you're using an App Password, not your regular Gmail password.",
                    "help": "Generate App Password at: https://myaccount.google.com/apppasswords",
                    "details": error_msg,
                    "troubleshooting": [
                        "1. Enable 2-Factor Authentication on your Gmail account",
                        "2. Go to https://myaccount.google.com/apppasswords",
                        "3. Select 'Mail' as the app and generate password",
                        "4. Copy the 16-character password (no spaces)",
                        "5. Update SENDER_PASSWORD in your .env file",
            ...
            return {
                "success": False,
                "error": f"Cannot connect to SMTP server {self.smtp_server}:{self.smtp_port}",
                "details": str(e),
                "troubleshooting": [
    ...
    def _get_recommendations_html(self, alert_data: Dict) -> str:
        ...
        if alert_type == 'peak_power':
            ...
            recommendations = [
                "This is a test alert to verify your email configuration",
                "If you received this email, your alert system is working correctly",
                "You can now configure real alerts for your energy monitoring system"
            ]
    ...
    def send_alert_email(self, alert_data: Dict, recipients: List[str]) -> Dict:
        ...
        try:
            ...
            print(f"General email sending error: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'sent_to': []
...
For support, please contact your system administrator.
    ...
    def send_test_email(self, recipient: str) -> Dict:
        ...
        return self.send_alert_email(test_alert_data, [recipient])
